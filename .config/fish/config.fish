...
end # Gitignore generator
function wttr
    curl "https://wttr.in/$argv?1F"
end # Weather
function cheat
    curl cheat.sh/(string join + $argv) | bat
end # Cheat sheet!
which cht.sh &>/dev/null && function cheat
    cht.sh $argv | bat
end
function mkcd
    mkdir -p $argv[1] && cd $argv[1]
end # Make a directory and enter it
